...
exports.registerUsers = (req, res, next) => {
    mysql.getConnection((error, conn) => {
        if(error) { console.log('conn'); return res.status(500).send({ error: error }) }
        const params = {
        ...
        conn.query('SELECT USR_LOGINNAME FROM USERS WHERE USR_LOGINNAME = ?', [req.body.USR_LOGINNAME], (error, results) => {
            if(error) { console.log('query'); return res.status(500).send({ error: error }) }
            if(results.length > 0){
                res.status(409).send({ mensagem: 'Usuário já cadastrado'})
            } else {
                bcrypt.hash(req.body.USR_PASSWORD, 10, (errBcrypt, hash) => {
                    if(errBcrypt){ console.log('Bcrypt'); return res.status(500).send({ error: errBcrypt }) }
                    S3.upload(params, function(err, data) {
                        if (err) { console.log(data.Location); return res.status(500).send({ error: err }) }
                        conn.query(
                            'CALL REGISTER_USERS(?, ?, ?, ?, ?, ?, ?, ?);', 
                            [
                            ...
                            (error, result, field) => {
                                conn.release();
                                if(error) { console.log('db'); res.status(500).send({ error: error }) }
            ...
            S3.upload(params, function(err, data) {
                ...
                conn.query(
                    ...
                    [
                        ...
                        req.body.USR_PHONENUMBER, req.body.USR_DATEBIRTHDAY, data.Location
                    ],
                    (error, result, field) => {
                        conn.release();
